...
2. Checkout the `main-juice` branch
...
Running the benchmarks requires Docker so you will need to have Docker setup on your machine.  See instructions in [../NVIDIA/README.md](../NVIDIA/README.md) for more details.

If you also want to run the NVIDIA benchmarks for comparison you will need [nvidia-docker](https://github.com/NVIDIA/nvidia-docker) installed too.  You don't need this to only run the Juice benchmarks which attach their GPUs dynamically.
...
Copy the archive for the Juice build that you want to use, e.g. `juice-gpu-linux.tar.gz`, to `closed/Juice/docker/common`.  You can obtain this archive using `juice release download <version>` if you already have Juice installed and know the version that you want.  Otherwise you can download from the Welcome screen after [logging in to the Juice web portal](https://app.juicelabs.co/login).
...
### Share GPUs using Juice

Share GPUs using Juice from another system or from outside the Docker container.  For example by running `juice agent run <pool>` or having GPUs shared through a service.

The rest of this guide assumes that this GPU is the only shared GPU available to your Juice user and so `juice run` without additional parameters will select the correct GPU.  Select specific GPUs with the `--gpu-ids` or `--pool-ids` parameters to `juice run`.  Select multiple GPUs by passing multiple comma-separated ids to `--gpu-ids` or use the `--num-gpus` flag.
